...
def test_proxy_add_group(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    try:
        ...
        if json['channelIds'] == "未綁定channel":
            req = proxyChannel()
            json['channelIds'] = [req.getAvailableChannel_auto(platToken=getPltLoginToken)]
...
def test_proxy_delete_group(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    if "存在groupId" in req_url:
        groupId = proxyGroup()
        req_url = req_url.replace("存在groupId",str(groupId.getExistGroup_auto(platToken=getPltLoginToken)))
...
def test_proxy_add_proxy(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    try:
        if json['proxyAccount'] == "不重複名稱":
            json['proxyAccount'] = json['proxyAccount']+str(random.randrange(99999))
    except :
        pass
    api = API_Conttoler()
    resp = api.HttpsClient(req_method,req_url,json,params,token=getPltLoginToken)
...
def test_proxy_proxy_commission(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    ...
    resp = api.HttpsClient(req_method,req_url,json,params,token=getPltLoginToken)
    clean = userManage()
    clean.cleanApproval(platToken=getPltLoginToken,optType="COMMISSION_CHANGE")
    assert resp.status_code == code_status ,resp.text
    assert keyWord in resp.text
...
def test_proxy_manage_approver_first(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    if "待審核訂單" in req_url :
        orderid = proxyManage()
        req_url = req_url.replace("待審核訂單",orderid.getFirstApprovalId(token=getPltLoginToken))
    if "一審完成訂單" in req_url :
        orderid = proxyManage()
        req_url = req_url.replace("一審完成訂單",orderid.getSecondApprovalId(token=getPltLoginToken))
    if "二審完成訂單" in req_url :
        orderid = proxyManage()
        req_url = req_url.replace("二審完成訂單",orderid.getSecondApprovalSuccessId(token=getPltLoginToken))
...
def test_proxy_manage_approver_second(test_case, req_method, req_url, scenario, json, params, code_status, keyWord,getPltLoginToken):
    if "待審核訂單" in req_url :
        orderid = proxyManage()
        req_url = req_url.replace("待審核訂單",orderid.getSecondApprovalId(token=getPltLoginToken))
    if "二審完成訂單" in req_url :
        orderid = proxyManage()
        req_url = req_url.replace("二審完成訂單",orderid.getSecondApprovalSuccessId(token=getPltLoginToken))

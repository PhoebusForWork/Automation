...
class User_wallet:
    ...
    def user_wallet_reset(balance="1000", user_id="29"):
        ...
        sql = - \
            f"update wallet.vs_wallet set balance = {balance} ,freeze_balance=0 where user_id = {user_id} ;"
    ...
    def user_wallet_deposit_check(user_id="29", check_amount=100, channel="AWC"):
        ...
        game_transfer = - \
            f"select type,amount from wallet.vs_game_transfer where user_id = {user_id} order by create_time desc limit 1;"
        data = check.select_sql(game_transfer)
        Type, amount = data[0][0], data[0][1]
        game_wallet = - \
            f"select channel_code,balance,freeze_balance from wallet.vs_game_wallet where user_id = {user_id};"
    ...
    def user_wallet_withdraw_check(user_id="29", check_amount=100, channel="AWC"):
        ...
        Type, amount = data[0][0], data[0][1]
        game_wallet = - \
            f"select balance,freeze_balance from wallet.vs_wallet where user_id = {user_id};"
        data = check.select_sql(game_wallet)

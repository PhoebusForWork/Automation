...
class API_Conttoler:
    ...
    def _printresponse(self,response):     #印出回傳
        ...
        print(json.dumps(printR, sort_keys=True, indent=4, separators=(',', ': '),ensure_ascii=False)) 
    ...
    def HttpsClient(self,reqMethod,reqUrl,json,params,token=None): 
        ...
        if reqMethod == 'post':
            response = self.ps.post(platfrom_host+reqUrl,json=json,params=params)
        elif reqMethod == 'put':
            response = self.ps.put(platfrom_host+reqUrl,json=json,params=params)
        elif reqMethod == 'get':
            response = self.ps.get(platfrom_host+reqUrl,json=json,params=params)  
        elif reqMethod == 'delete':
            response = self.ps.delete(platfrom_host+reqUrl,json=json,params=params)  

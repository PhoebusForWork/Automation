...
class validation(WebAPI):
    ...
    def valid_captcha(self):  # 取得圖形驗證廠商
        response = self.request_session.get(web_host+"/v1/validation/captcha",
                               data={},
                               params={}
                               )
    ...
    def valid_email(self, device=None, requestType=None):  # 發送郵箱驗證訊息
        response = self.request_session.post(web_host+"/v1/validation/email",
                                json={
                                    "device": device,
    ...
    def valid_sms(self, device=None, requestType=None):  # 發送短信驗證訊息
        response = self.request_session.post(web_host+"/v1/validation/sms",
                                json={
                                    "device": device,
                                    "requestType": requestType,
                                },
    ...
    def valid_voice(self, device=None, requestType=None):  # 發送短信驗證訊息
        response = self.request_session.post(web_host+"/v1/validation/voice",
                                json={
                                    "device": device,
                                    "requestType": requestType,
                                },

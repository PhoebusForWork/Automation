...
class WalletManage(PlatformAPI):
    ...
    # 限制流水清零
    def withdraw_limit_water_clear(self, plat_token=None, userId=None, remark=None):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
    ...
    # 獲取提現流水匯總
    def get_withdraw_water_total(self, plat_token=None, userId=None):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
    ...
    def get_withdraw_limit_water_clear_pending_list(
            self, plat_token=None, userId=None, orderId=None, orderType=None, adminName=None,
            waterStatus=None, limitGameType=None, limitGameCode=None, page=None, size=None
    ):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
...
class WalletGameTransfer(PlatformAPI):
    ...
    # 後台轉入指定渠道
    def deposit(self, plat_token=None, userId=None, channelCode=None, amount=None):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
...
class WalletGameTransferFailed(PlatformAPI):
    ...
    def _create_failed_transfer_record():
        cs_wallet = Wallet()
        data = cs_wallet.login(deviceId="345", username='AutoTester', password="abc123456").json()['data']
    ...
    # 轉帳異常處理列表
    def get_failed_list(
            self, plat_token=None, timeType=None, startTime=None, endTime=None, failedTransferStatus=None,
            operator=None, fromChannelCode=None, toChannelCode=None, tradeId=None, page=None, size=None
    ):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
    ...
    # 手動處理異常轉帳
    def trade_manual_result(self, plat_token=None, tradeId=None, result=None, remark=None):
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})

...
class User(PlatformAPI):
    ...
    def get_user_list(self,  # 查詢客戶列表
        ...
        response = self.request_session.get(platform_host+"/v1/user",
                               json={},
                               params={
                                   "from": From, "to": to,
    ...
    def user_params(self,  # 客戶列表查詢條件
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/params",
                               json={},
                               params={}
                               )
        self.print_response(response)
    ...
    def get_user_info(self,  # 查詢客戶資訊
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/{}".format(userId),
                               json={},
                               params={}
                               )
        self.print_response(response)
    ...
    def get_user_amount(self,  # 查詢客戶財務資訊
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/{}/account".format(userId),
                               json={},
                               params={}
                               )
        self.print_response(response)
    ...
    def user_remark(self,  # 更新客戶備註(不需審核)
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/{}/remark".format(userId),
                               json={
            "remark": remark,
        },
            params={}
    ...
    def user_really_name(self,  # 更新客戶真實姓名(不需審核)
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/{}/reallyName".format(userId),
                               json={
    ...
    def set_white_list(self,  # 設置客戶撞庫白名單
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/{}/isWhiteList".format(userId),
                               json={},
                               params={
    ...
    def user_convert_proxy(self,  # 開通代理角色
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/convertProxy",
                               json={
                                   "userId": userId,
                               },
                               params={}
    ...
    def username_validate(self,  # 校驗會員帳號是否存在
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.post(platform_host+"/v1/user/username/validate",
                                json={
                                    "usernames": usernames
                                },
                                params={}
    ...
    def user_lock_status(self,  # 申請鎖定 lockStatus(鎖定類別) LOGIN：登入, RECHARGE：充值, WITHDRAW：提領, TRANSFER：轉帳
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.post(platform_host+"/v1/user/lockStatus",
                                json={
                                    "userIds": userIds,
                                    "userLockTypeList": [
                                        {
    ...
    def edit_user_vip(self,  # 修改客戶VIP層級
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/userVip/{}".format(userId),
                               json={},
                               params={
            "userId": userId,
            "vipId": vipId,
    ...
    def get_user_vip_info(self,  # 查詢客戶VIP層級
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/userVip",
                               json={},
                               params={
                                   "userName": userName,
                               }
...
# --------------------------------------------------------------------------------------------
    ...
    def risk_analysis_same_ip(self,  # 風險套利重複ip
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/{}/risk/analysis/same/ip".format(userId),
                               json={},
                               params={
            "page": page,
            "size": size,
    ...
    def risk_analysis_arbitrage(self,  # 風險套利分析查詢
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/{}/risk/analysis/arbitrage".format(id),
                               json={},
                               params={
            "from": From, "to": to,
    ...
    def login_info(self,  # 登入日誌
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/{}/login/info".format(id),
                               json={},
                               params={
            "from": From, "to": to,
    ...
    def login_stat(self,  # 客戶登入統計
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/login/stat/{}".format(userId),
                               json={},
                               params={}
                               )
        self.print_response(response)
...
class UserManage(PlatformAPI):
    ...
    def get_user_manage_list(self,  # 查詢審批列表
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/manage",
                               json={},
                               params={
                                   "from": From, "to": to,
    ...
    def get_user_manage_query_params(self,  # 查詢審批參數
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/manage/query/params",
                               json={},
                               params={},
                               )
        self.print_response(response)
    ...
    def get_user_manage_log(self,  # 後台操作記錄
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/manage/log/{}".format(userId),
                               json={},
                               params={
            "from": From, "to": to,
    ...
    def first_approval(self,  # 一審 審核狀態 1:核准 / 2:駁回
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/manage/{}/first/approval".format(id),
                               json={
            "status": status,
            "remark": remark,
        },
    ...
    def second_approval(self,  # 二審 第二審批狀態 4:核准 / 5:駁回
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/manage/{}/second/approval".format(id),
                               json={
            "status": status,
            "remark": remark,
        },
    ...
    def user_manage_parent(self,  # 修改上級代理
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/manage/parent",
                               json={
                                   "userId": userId,
...
class UserVip(PlatformAPI):
    ...
    def add_vip(self,  # 新增VIP層級
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.post(platform_host+"/v1/user/vip/config",
                                json={
                                    "name": name,
    ...
    def edit_vip(self,  # 編輯VIP層級
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/vip/config/{}".format(vipId),
                               json={
            "name": name,
            "regStartTime": regStartTime,
            "regEndTime": regEndTime,
    ...
    def edit_vip_only(self,  # 編輯VIP專享
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.put(platform_host+"/v1/user/vip/config/{}/isVip".format(vipId),
                               json={},
                               params={
            "vipId": vipId,
            "isVip": isVip,
    ...
    def get_vip_info(self,  # 獲取VIP配置
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/vip/config",
                               json={},
                               params={}
                               )
        self.print_response(response)
    ...
    def get_vip_list(self,  # 獲取VIP列表
        ...
        if plat_token is not None:
            self.request_session.headers.update({"token": str(plat_token)})
        response = self.request_session.get(platform_host+"/v1/user/vip/config/mapList",
                               json={},
                               params={}
                               )
        self.print_response(response)

...
echo "###### $RECEIVRE_ITEM ######" && echo "## Rebuild ##"
# PGPASSWORD=$PG_MANAGE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_MANAGE_USER -d $RECEIVRE_ITEM  <<EOFSQL
PGPASSWORD=$RECEIVE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_PLT_JAVA_APP_USER -d $RECEIVRE_ITEM  <<EOFSQL
    DROP SCHEMA $RECEIVRE_ITEM cascade;
    CREATE SCHEMA $RECEIVRE_ITEM;
    ...
    GRANT CREATE ON SCHEMA $RECEIVRE_ITEM TO plt_rd_role;
...
PGPASSWORD=$RECEIVE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_PLT_JAVA_APP_USER -d $RECEIVRE_ITEM  <<EOFSQL
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA $RECEIVRE_ITEM GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO plt_rd_role;
EOFSQL
}
...
echo "###### $RECEIVRE_ITEM ######" && echo "## Rebuild ##"
# PGPASSWORD=$PG_MANAGE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_MANAGE_USER -d $RECEIVRE_ITEM  <<EOFSQL
PGPASSWORD=$RECEIVE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_CS_JAVA_APP_USER -d $RECEIVRE_ITEM  <<EOFSQL
    DROP SCHEMA $RECEIVRE_ITEM cascade;
    CREATE SCHEMA $RECEIVRE_ITEM;
    REVOKE CREATE ON SCHEMA public FROM PUBLIC;
    REVOKE ALL ON DATABASE $RECEIVRE_ITEM FROM PUBLIC;
    ...
    GRANT CREATE ON SCHEMA $RECEIVRE_ITEM TO cs_rd_role;
...
PGPASSWORD=$RECEIVE_PASSWORD psql -h $RECEIVE_ADDRESS -p $POSTGRES_PORT -U $PG_CS_JAVA_APP_USER -d $RECEIVRE_ITEM  <<EOFSQL
    ...
    ALTER DEFAULT PRIVILEGES IN SCHEMA $RECEIVRE_ITEM GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO cs_rd_role;
EOFSQL
}
...
run() {
  ...
  if [ "$TEXT" == "CS" ]; then
    ...
    for item in ${ARRAY[@]}; do
      cs_syntax $PASS_ADDRESS $PASS_PASSWORD $item
    done
    ...
    for item in ${ARRAY[@]}; do
      plt_syntax $PASS_ADDRESS $PASS_PASSWORD $item
    done
